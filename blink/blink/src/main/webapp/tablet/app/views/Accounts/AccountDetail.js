synchronicity.views.AccountDetail = Ext.extend(Ext.TabPanel,{
	id : "synchronicity.views.accountDetail",
	fullscreen : true,
	tabBar : {
		dock : 'bottom',
		ui : 'light',
	...
	items : [ new synchronicity.views.AccountActivity(),
			new synchronicity.views.AccountContact(),
			new synchronicity.views.AccountOpportunity() ],
	...
	initComponent : function() {
		synchronicity.views.AccountDetail.superclass.initComponent.apply(this, arguments);
	},
	...
	getNavigationBarItems : function() {
		...
		this.searchIcon = new Ext.Button(
				{
					...
					handler : function() {
						...
						if (panelObject.getActiveItem().getId() == 'synchronicity.views.accountActivity') {
							Ext.dispatch({
		                         controller: synchronicity.controllers.activity,
		                         action: 'showSearchBox',
		                         baxObject: this,
		                         accountId: panelObject.getActiveItem().accountList.store.first().get('guid')
		                     });
						}
						else if (panelObject.getActiveItem().getId() == 'synchronicity.views.accountContact') {
							Ext.dispatch({
		                         controller: synchronicity.controllers.contact,
		                         action: 'showSearchBox',
		                         baxObject: this,
		                         accountId: panelObject.getActiveItem().accountList.store.first().get('guid'),
		                         contactIds: panelObject.getActiveItem().accountList.store.first().get('contactIds')
		                     });
						}
						else if (panelObject.getActiveItem().getId() == 'synchronicity.views.accountOpportunity') {
							Ext.dispatch({
		...
		this.addIcon = new Ext.Button(
				{
					...
					handler : function() {
						if (panelObject.getActiveItem().getId() == 'synchronicity.views.accountActivity') {
									...
									itemtap: function(list,index,item,e){
										...
										if (list.getStore().getAt(index).get('option') == 'Appointment') {
											Ext.dispatch({
						                         ...
						                         mode: 'add',
						                         type: 'APPOINTMENT',
						                         accountGuid: panelObject.getActiveItem().accountList.getStore().getAt(0).get('guid')
						                    });
										}
										else {
											Ext.dispatch({
							...
							var accountId = panelObject.getActiveItem().accountList.store.getAt(0).get('id');
							var accountGuid = panelObject.getActiveItem().accountList.store.getAt(0).get('guid');
								...
								dockedItems: [{xtype: 'toolbar', 
											   items: [{text: 'New', 
											   			...
											   			Ext.dispatch({
									                         ...
									                         mode: 'add',
									                         accountId: panelObject.getActiveItem().accountList.getStore().getAt(0).get('id'),
									                         accountGuid: panelObject.getActiveItem().accountList.getStore().getAt(0).get('guid')
											   				});
											   		  	}},
											   		  	{xtype: 'spacer'},
		...
		this.backButton = {
			...
			ui : 'back',
			handler : function() {
				Ext.getCmp('synchronicity.views.accountDetail').fireEvent(
																	'navigateTo',
																	null,
																	Ext.getCmp('synchronicity.views.accountList'),
																	false);
				panelObject.setActiveItem(panelObject.items.first(),'slide');
			}
		};
		this.navigationBarItems = [ this.backButton, {xtype : 'spacer'}, this.addIcon, this.searchIcon ];
		return this.navigationBarItems;
	},

	initialize : function(record) {
		var store = Ext.getStore('synchronicity.stores.transientAccountSingle');
